...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var songName:Array<String> = SONG.song.split("-");
		for(song in songName)
		{
			var songArray:Array<String> = song.split("");
			for(s in songArray)
			{
				if(songArray.indexOf(s)==0)
				{
					s = s.toUpperCase();
				}
				songInfo += s;
			}
			if(songName.indexOf(song)< songName.length-1)
			{
				songInfo += " ";
			}
		...
		timeTxt.setFormat(Paths.font("mars.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("mars.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("mars.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		DiscordClient.changePresence(detailsText, songInfo + " (" + storyDifficultyText + ")", iconP2.getCharacter());
		#end
  ...
  public function clearShaderFromCamera(cam:String){
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, songInfo + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength);
		#end
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, songInfo + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(detailsText, songInfo + " (" + storyDifficultyText + ")", iconP2.getCharacter());
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			DiscordClient.changePresence(detailsPausedText, songInfo + " (" + storyDifficultyText + ")", iconP2.getCharacter());
		}
		#end
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Game Over - " + detailsText, songInfo + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			// This is where the unlocking in songs will comence
			// if (WeekData.getCurrentWeek().hideFreeplay == true)
			// {
			// 	WeekData.getCurrentWeek().hideFreeplay == false;
			// }

			if(!Paths.getAvailability(Paths.formatToSongPath(SONG.song)))
			{
				Paths.setAvailability(Paths.formatToSongPath(SONG.song), true);
			}

			if(!Paths.getMystery(Paths.formatToSongPath(SONG.song)))
			{
				Paths.setMystery(Paths.formatToSongPath(SONG.song), true);
			}
