...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		scoreText = new FlxText(infobox.x + 50, infobox.y +  250, 0, "SCORE: 49324858", 36);
		scoreText.setFormat("Determination Sans Web Regular", 32);


		txtWeekTitle = new FlxText(FlxG.width * 0.7, 10, 0, "", 32);
		txtWeekTitle.setFormat(Paths.font("undertale.otf"), 32, FlxColor.WHITE, CENTER);
		txtWeekTitle.size = 60;
		
		var rankText:FlxText = new FlxText(0, 10);
		rankText.text = 'RANK: GREAT';
		rankText.setFormat(Paths.font("determination.ttf"), 32);
		...
		DiscordClient.changePresence("Story Mode? Horray!", null);
		#end
		...
		var grid:FlxSprite = new FlxSprite().loadGraphic(Paths.image('battleGrid'));
		grid.antialiasing = ClientPrefs.globalAntialiasing;
		add(grid);
		grid.setGraphicSize(1280,516);
		grid.y = 150;
		grid.screenCenter(X);
		...
		// difficultySelectors.setPosition();
		...
		// add(rankText);
		add(scoreText);
		scoreText.x = infobox.x + 50;
		scoreText.y = infobox.y + 120;
		scoreText.size = 40;

		add(txtWeekTitle);

		txtWeekTitle.y = infobox.y + 40;
	...
	override function update(elapsed:Float)
	{
		...
		txtDifficulty.text = "* Difficulty: " + lastDifficultyName;
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		// txtDifficulty.text = CoolUtil.difficulties[curDifficulty];
	...
	function updateText()
	{
		...
		// txtDifficulty.text = CoolUtil.difficultyString();
