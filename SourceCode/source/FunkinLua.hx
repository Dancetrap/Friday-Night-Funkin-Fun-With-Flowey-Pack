...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "unlockSong", function(songName:String) {
			Paths.setAvailability(Paths.formatToSongPath(songName), true);
			// luaTrace('Song Name does not exist', false, true);
		});

		Lua_helper.add_callback(lua, "lockSong", function(songName:String) {
			Paths.setAvailability(Paths.formatToSongPath(songName), false);
			// luaTrace('Song Name does not exist', false, true);
		});

		Lua_helper.add_callback(lua, "revealSong", function(songName:String) {
			Paths.setMystery(Paths.formatToSongPath(songName), true);
			// luaTrace('Song Name does not exist', false, true);
		});

		Lua_helper.add_callback(lua, "hideSong", function(songName:String) {
			Paths.setMystery(Paths.formatToSongPath(songName), false);
			// luaTrace('Song Name does not exist', false, true);
		});
