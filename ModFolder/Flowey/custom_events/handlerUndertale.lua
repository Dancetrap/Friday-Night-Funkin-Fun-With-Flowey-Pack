...
function bullSpawn(type, xx, yy, hsp, vsp, dmg, ang, exArg, exArg2)
	...
	-- makeLuaSprite(name, 'ut/flowey/'..spr, boxX + xx, boxY + yy);
...
function floweySpawn(type, xx, yy, hsp, vsp, dmg, ang, exArg, exArg2)
	hsp = hsp or 0;
	vsp = vsp or 0;
	dmg = dmg or 1;
	ang = ang or 0;
...
function animatedBullSpawn(type, xx, yy, hsp, vsp, dmg, ang, exArg, exArg2)
	hsp = hsp or 0;
	vsp = vsp or 0;
	dmg = dmg or 1;
	ang = ang or 0;
	...
	-- makeLuaSprite(name, 'ut/bullets/'..spr, boxX + xx, boxY + yy);
	makeAnimatedLuaSprite(name, 'ut/flowey/'..spr, boxX + xx, boxY + yy)
	luaSpriteAddAnimationByPrefix(name, name..'attack', spr..' idle')
	luaSpritePlayAnimation(name, name..'attack')
	-- makeLuaSprite(name, 'ut/flowey/'..spr, boxX + xx, boxY + yy);
	setProperty(name..'.offset.x', getProperty(name..'.width') / 2);
	setProperty(name..'.offset.y', getProperty(name..'.height') / 2);
...
function bullStep(i)
	...
	elseif (tp == 'vine') then
		local b = floweySpawn('vineattack', 0, 0, 0, 0, bProp[n..'.dmg'], 0);
		bProp[b..'.ang'] = bProp[n..'.ang'];
		bProp[n..'.blast'] = b;
		bProp[b..'.exArg'] = bProp[n..'.exArg'];
...
function onCreate()
	...
	makeLuaSprite('p', 'ut/soul8bit', boxX, boxY);
	-- setProperty('p.offset.x', 225);
	-- setProperty('p.offset.y', -100);
	setProperty('p.offset.x', 8);
	setProperty('p.offset.y', 8);
	setProperty('p.scale.x', 2);
	setProperty('p.scale.y', 2);
	-- makeLuaSprite('p', 'ut/heartTilt', boxX, boxY);
	-- setProperty('p.offset.x', 8);
	-- setProperty('p.offset.y', 8);
	-- setProperty('p.scale.x', 1);
	-- setProperty('p.scale.y', 1);
...
function atkIni()
	...
	elseif(k == 'circle')then
		wav = 2;
	elseif(k == 'pellet')then
		targetH = 576;
		targetW = 1024;
	elseif(k == 'wave')then
		targetH = 576;
		targetW = 1024;
	elseif(k == 'spiral')then
		targetH = 576;
		targetW = 1024;
	end
...
function atkStep()
	...
	if(k == 'spiral') then
	-- local r = math.random(0,450);
	if (atkTimer % 6 == 0) then
			if (wav < 3) then wav = wav + 0.2 end
			local rn = 100;
			local a = atkTimer*wav;
			-- local r = a - atkTimer*wav;
			local bx = math.cos(math.rad(a)) * rn;
			local by = -math.sin(math.rad(a)) * rn;
			...
			-- speed for each would be the position times -1 divided by the radius

			local pa = a + 180;
			-- The distance is 450, so it must

			-- if(pa < rc + 450) then
			animatedBullSpawn('friendliness-pellets', bx, by, sx, sy, 1, pa, 0.5);
	...
	if (atkTimer % 20 == 0) then
			...
			-- bullSpawn('boneV2', -500, rn - sp, 7, 0);
			-- bullSpawn('boneV2', -500, rn + sp, 7, 0);
			animatedBullSpawn('friendliness-pellets', -500, math.random(-288, 288) - math.random(0,100), math.random(4,12), 0);
			animatedBullSpawn('friendliness-pellets', -500, math.random(-288, 288) + math.random(0,100), math.random(4,12), 0);
			animatedBullSpawn('friendliness-pellets', 500, math.random(-288, 288) - math.random(0,100), math.random(4,12) * -1, 0);
			animatedBullSpawn('friendliness-pellets', 500, math.random(-288, 288) + math.random(0,100), math.random(4,12) * -1, 0);
		end
	end
	...
	-- local r = math.random(0,450);
	if (atkTimer % 6 == 0) then
			if (wav < 2) then wav = wav + 0.02 end
			local rn = 300;
			local a = atkTimer*wav + rc;
			-- local r = a - atkTimer*wav;
			local bx = math.cos(math.rad(a)) * rn;
			local by = -math.sin(math.rad(a)) * rn;
			...
			-- The distance is 450, so it must
			-- if(pa < rc + 450) then
			animatedBullSpawn('friendliness-pellets', bx-1200, by, 10, 1, 1, pa, 0.5);
			animatedBullSpawn('friendliness-pellets', bx+1200, by-100, -10, 0, 1, pa, 0.5);
			-- end
		end
	end

	if(k == 'makeCircle') then
	end
...
function onEvent(name, value1, value2)
	...
	elseif (name == 'vineAttack') then
			local rn = 200;
			local a = math.random(0, 360);
			local bx = math.cos(math.rad(a)) * rn;
			local by = -math.sin(math.rad(a)) * rn;

			local pa = point_direction(bx, by, 10, 100);
...
--    return {
--       local parent = makeShape(newx, newy),
--       local radius = newradius,

--       -- accessors for the radius
--       local getRadius = function (self) return self.radius end,
--       local setRadius = function (self, newradius) self.radius = newradius end,
